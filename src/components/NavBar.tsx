...
export default function NavBar(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="animate-welcome pointer-events-auto mt-3 flex max-h-[min(calc(100vh-4.5rem),42rem)] w-[min(92vw,44rem)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950/90 text-slate-100 shadow-2xl shadow-black/60 ring-1 ring-black/40 backdrop-blur-xl">
          {/* Professional header strip: subtle gradient + brand mark + close */}
          <div className="flex items-center justify-between border-b border-white/5 bg-gradient-to-r from-slate-900 via-slate-900 to-slate-900/70 px-4 py-2">
            <div className="flex items-center gap-2">
              <span className="flex h-5 w-5 items-center justify-center rounded-lg bg-amber-400/15 text-amber-300">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
          ...
          <div className="flex flex-wrap gap-1 px-4 pt-2.5">
            <div className="flex flex-wrap gap-1 rounded-full border border-white/10 bg-white/5 p-0.5">
              {TAB_LABELS.map((t) => (
                <button
                  key={t.value}
                  type="button"
              ...
              {columnOrder.map((key) => {
                ...
                return (
                  <button
                    ...
                    className={`group flex flex-col items-start gap-1 rounded-xl border p-2.5 text-left transition-all duration-200 ${
                      active
          ...
          {/* Settings — a single, professionally grouped panel: controls and
              key bindings first (desktop only), then sound/graphics, touch & UI,
              and world. */}
          {tab === 'settings' && (
            <div className="space-y-2.5">
              {deviceType !== 'mobile' && <ControlsPanel />}
          ...
          {/* Footer strip */}
          <div className="flex items-center justify-between border-t border-white/5 bg-slate-900/60 px-4 py-1.5 text-[10px] font-semibold text-white/35">
